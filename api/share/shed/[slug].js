...
module.exports = async function handler(req, res) {
  ...
  var pageUrl = domain + '/sheds-for-sale.html';

  try {
    var sheds = await getAllSheds();
    var s = sheds.find(function(x) { return x.slug === slug && x.available; });

    if (!s) {
      return res.redirect(302, pageUrl);
    }

    var title = s.name + ' - $' + (s.sale_price || 0).toLocaleString('en-US');
    var description = (s.size || '') + ' | Free Delivery & Setup | North Florida Building Solutions | Call (386) 350-1047';
    ...
    var html = '<!DOCTYPE html>\n<html>\n<head>\n' +
      ...
      '<meta property="og:url" content="' + esc(domain + '/api/share/shed/' + slug) + '">\n' +
